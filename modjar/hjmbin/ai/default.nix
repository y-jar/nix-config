...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: hjem: AI tools (opencode) — port of usrbin/ai[cringe] (home-manager).
# -=-=-=-=-=-=-=-=-=-=-=
# Writes opencode config via the hjemDotfiles mechanism (wired in
# modjar/hjemkey.nix):
...
# Packages (opencode CLI/GUI, uv, lmstudio) live in modjar/hjmbin/packages.nix
# under the same ai.* gates. The local llama.cpp server is system-side
# (sysSettings.ai, modjar/sysbin/ai[cringe]).
# -=-=-=-=-=-=-=-=-=-=-=
{
  config,
  lib,
...
in
{
  config = lib.mkIf (hjm.enable && hjm.opencode.enable) {
    hjemDotfiles = {
      opencodeJson = json.generate "opencode.json" (
        { "$schema" = "https://opencode.ai/config.json"; } // settings
      );
      opencodeContext = pkgs.writeText "AGENTS.md" context;
      opencodeAgents = lib.mapAttrs (name: text: pkgs.writeText "${name}.md" text) agents;
    };
  }; # end of config
}
