...

{
  config = {
    ...
    sysset = {
      # =============[users]
      mainUser = "fesur"; # primary user (gets hjem)
      users = [ "fesur" ]; # all users (main + any guests)
      adminUsers = [ "fesur" ]; # users with sudo access
      userDescriptions = {
        fesur = "fesur";
      ...
      mango.enable = true; # ~5mib sets mango (mangowm)
      cosmic = {
      ...
      gaming = {
        drivers = {
          enable = false; # ~1.5gib - Vulkan + Mesa
        ...
        codecs.enable = false; # ~codecs - GStreamer + VA-API/Vulkan debug tools
      ...
      server = {
        ...
        jellyfin = {
          ...
          juser = "fesur"; # sets jellyfin user for perms for file access
        }; # end of jellyfin
        sleepyjar = {
