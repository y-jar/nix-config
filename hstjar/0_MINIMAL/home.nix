...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: Host 0_MINIMAL: lean user-level toggle sheet (usrSettings).
# -=-=-=-=-=-=-=-=-=-=-=
...

{
  config = {
    ...
    usrSettings = {
      ...
      mango.enable = mangoEnable; # default WM on this minimal host
      shelljar.enable = false;
      ...
      terminal.enable = true; # foot
      terminal.font = "IntoneMono Nerd Font";
      ...
      ai.enable = aiEnable;
      ai.opencode.enable = true; # opencode (NON-local models only)
      ai.lmstudio.enable = false; # no LM Studio (saves ~2.3GiB)
