...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: Niri KDL config generation (hjem side).
# -=-=-=-=-=-=-=-=-=-=-=
# =-=-=[niri] =-=-=
# Copies the KDL config files (from liijar/wmconfigs/niri, shared with the
# home-manager side). The desktop shell spawn + launcher/settings binds are
# gated on usrset.niri.{shelljar,noctalia}.enable (mirrors the home-manager
