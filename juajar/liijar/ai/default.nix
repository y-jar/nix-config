...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: AI tools (opencode): config file writes + packages.
# -=-=-=-=-=-=-=-=-=-=-=
# Writes opencode config into the user `files` option:
...
let
  ...
  # opencode + uv + lmstudio under the ai.* fine toggles
  aiPackages =
    lib.optionals (hjm.enable && hjm.opencode.enable) [
    ...
    ++ lib.optionals (hjm.enable && hjm.lmstudio.enable) [
  ...
  agents = {
    nix-helper = ''
      ...
      - New app = a dir in `juajar/liijar/<app>/` with a single `default.nix`
...
{
  config = {
    # opencode + uv + lmstudio under the ai.* gates (see aiPackages)
    packages = aiPackages;
