...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: Voxtype push-to-talk dictation + transcript (hjem side).
# =-=-=[dictation] =-=-=
# Packages come from ./shared.nix. hjem does not manage user systemd units,
# so the daemon + model bootstrap are shipped as plain unit files under
# ~/.config/systemd/user/ and enabled on login via the dirSetup (.profile)
# bootstrap (the bus is declared in liijar/profile-bus.nix).
# =-=-=[end dictation] =-=-=
{
  config,
  lib,
