...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: fcitx5/mozc/hangul input methods.
# =-=-=[inputmethods] =-=-=
# Japanese/Korean input: fcitx5 + mozc/hangul. Direct dotfile writes
# (fcitx5 profile/config/conf files + mozc config1.db) and the IME
...
let
  ...
  imPackages = [ fcitx5Package ]; # end of imPackages
...
in
{
  config = lib.mkIf (cfg.japanese.enable || cfg.korean.enable) {
    ...
    # IME environment (lands in the user environment -> .profile via loadEnv)
    ...
    # silent no-op (the user submodule swallows system-scope options), so
