...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: hjem zsh: zshrc + fzfrc generation.
# -=-=-=-=-=-=-=-=-=-=-=
# Aliases + shell functions are single-sourced in juajar/liijar/shell/
# and shared with the home-manager backend (liijar/zsh/hm.nix).
# -=-=-=-=-=-=-=-=-=-=-=
{
  config,
  lib,
...
let
  ...
  # attrset of aliases -> `alias k=v` lines (mirrors hm programs.zsh.shellAliases)
  ...
  zshrc = pkgs.writeText "zshrc" ''
    ...
    # functions (shared with home-manager hosts via liijar/shell)
    ${functions}

    # aliases (shared with home-manager hosts via liijar/shell)
    ${aliasLines}
  '';
