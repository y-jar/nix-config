...
# . ▀▀ : ╃
# -=-=-=-=-=-=-=-=-=-=-=
# goal: `jn` (jar's nix shell) - a playful interactive shell launcher (hjem side).
...
let
  # Per-host gates.
  unfreeAllowed = osConfig.sysset.unfree.enable or false;
  yoinkEnabled = osConfig.sysset.UseNixPkgsYoinks.enable or false;
  ...
  jn = pkgs.writeShellScriptBin "jn" ''
    ...
    case "$choice" in
      *moss*)
        ...
        printf '%s\n' "These apps were installed: ${mossLabel}"
        printf '%s\n' "Have a fun time in the jar!"
