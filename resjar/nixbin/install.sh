...
auto_partition() {
    ...
    # For BIOS+XFS: GRUB 2.12's xfs module can't read modern XFS to load
    # kernel/initrd. Create a separate 1G ext4 /boot so GRUB reads from ext4.
    local xfs_boot_part_dev=""
    if [ "$boot_mode" = "BIOS" ] && [ "$fs_type" = "xfs" ]; then
        script+=",1G,L"$'\n'
        xfs_boot_part_dev="/dev/${disk_base}${part_suffix}${part_num}"
        part_num=$((part_num + 1))
    fi
    ...
    # For BIOS+XFS: 16G to leave space for 1G /boot (20G disk).
    local root_size="18G"
    ...
    [ -n "$xfs_boot_part_dev" ] && export INSTALLJAR_AUTO_BOOT_PART="$xfs_boot_part_dev"
...
gen_vm_hardware_config() {
    ...
    if [ -n "$boot_part" ]; then
        if [ "$boot_mode" = "UEFI" ]; then
            ...
            out+="$(gen_fs_entry "/boot" "$boot_part" "ext4" "")"$'\n'
        fi
    fi
